...
impl ReqwestTransport {
    ...
    fn build(&self, req: Request) -> Result<CodexRequestBuilder, TransportError> {
        ...
        if let Some(body) = body {
            // Serialize JSON body to bytes first
            ...
            if compression != RequestCompression::None {
                ...
                let pre_compression_bytes = json_bytes.len();
                ...
                let (compressed, content_encoding) = match compression {
                    ...
                    RequestCompression::Zstd => (
                        zstd::stream::encode_all(std::io::Cursor::new(json_bytes), 3)
                            .map_err(|err| TransportError::Build(err.to_string()))?,
                ...
                builder = builder.headers(headers).body(json_bytes);
            }
        } else {
...
impl HttpTransport for ReqwestTransport {
    async fn execute(&self, req: Request) -> Result<Response, TransportError> {
        ...
        if let Some(body) = &req.body {
            tracing::debug!("Request body before build: {:?}", body);
        }
