...
impl Codex {
    ...
    pub(crate) async fn spawn(
        ...
        tracing::debug!(
            ...
            base_instructions.chars().take(60).collect::<String>()
        );
...
impl Session {
    ...
    async fn record_initial_history(&self, conversation_history: InitialHistory) {
        ...
        match conversation_history {
            ...
            InitialHistory::Resumed(resumed_history) => {
                ...
                info!("Seed usage info from the recorded rollout so UIs can show token counts");
    ...
    pub(crate) async fn build_initial_context(
        ...
        let (collaboration_mode, _base_instructions) = {
            let state = self.state.lock().await;
            (
                state.session_configuration.collaboration_mode.clone(),
    ...
    pub(crate) async fn update_token_usage_info(
        ...
        debug!("update_token_usage_info called: token_usage={}, model_context_window={:?}",
            if token_usage.is_some() { "Some" } else { "None" },
            turn_context.client.get_model_context_window());
        {
            let mut state = self.state.lock().await;
            if let Some(token_usage) = token_usage {
                debug!("update_token_usage_info: updating token info with tokens={}",
                ...
                debug!("update_token_usage_info: token_usage is None, skipping update");
            }
        }
        self.send_token_count_event(turn_context).await;
    }
    ...
    pub(crate) async fn send_token_count_event(&self, turn_context: &TurnContext) {
        ...
        if info.is_none() && rate_limits.is_none() {
            return;
        }
...
pub(crate) async fn run_turn(
    ...
    let connector_slug_counts = if false /* Apps feature removed */ {
    ...
    loop {
        ...
        .await
        {
            Ok(sampling_request_output) => {
                ...
                // Ensure token info is sent to TUI even if ResponseEvent::Completed wasn't received
                // (stream may close before that event is emitted)
                let model_window = turn_context.client.get_model_info().context_window;
                {
                    let mut state = sess.state.lock().await;
                    ...
                    if let Some(mut info) = current_info {
                        ...
                        state.set_token_info(Some(info));
                    } else if let Some(window) = model_window {
                        // Create token info with estimated count. estimate_token_count already
                        // includes base instruction tokens, so use it directly.
                        let total_tokens = estimated_token_count.unwrap_or(0);
...
async fn run_sampling_request(
    ...
    let mut mcp_tools = sess
        ...
        .or_cancel(&cancellation_token)
        .await?;
    let connectors_for_tools = if false /* Apps feature removed */ {
        let connectors = connectors::accessible_connectors_from_mcp_tools(&mcp_tools);
    ...
    loop {
        ...
        if !err.is_retryable() {
            error!("LLM request failed (non-retryable): {err:#}");
            return Err(err);
        }
        ...
        if retries < max_retries {
            ...
            error!("LLM request failed after {max_retries} retries: {err:#}");
            return Err(err);
        }
    }
}
    ...
    let outcome: CodexResult<SamplingRequestResult> = loop {
        ...
        match event {
            ...
            ResponseEvent::Completed {
                ...
                debug!("ResponseEvent::Completed: token_usage={}", if token_usage.is_some() { "Some" } else { "None" });
                if let Some(state) = plan_mode_state.as_mut() {
