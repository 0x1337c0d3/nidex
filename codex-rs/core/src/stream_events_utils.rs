...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(ctx.sess.as_ref(), item.clone()).await {
        ...
        Err(FunctionCallError::MissingLocalShellCallId) => {
            ...
            let call_id = call_id_from_item(&item).unwrap_or_default();
...
/// Extract the call_id from a ResponseItem so error responses can be paired
/// with their originating tool call in conversation history.
fn call_id_from_item(item: &ResponseItem) -> Option<String> {
    match item {
