    ...
    match provider {
        ...
        _ => {
            return Err(std::io::Error::new(
                ...
                format!(
                    "Invalid OSS provider '{provider}'. Must be one of: {OLLAMA_OSS_PROVIDER_ID}, {OLLAMA_CHAT_PROVIDER_ID}"
                ),
            ));
        }
    }
...
pub struct ConfigToml {
    ...
    /// Preferred OSS provider for local models, e.g. "ollama", or "ollama-chat".
...
"#,
        ...
        let project_config_dir = workspace.path().join(".nidex");
...
"#
    ...
    async fn set_model_updates_defaults() -> anyhow::Result<()> {
        ...
        ConfigEditsBuilder::new(codex_home.path())
            .set_model(Some("big-pickle"), Some(ReasoningEffort::High))
            .apply()
            .await?;
        ...
        assert_eq!(parsed.model.as_deref(), Some("big-pickle"));
...
"#,
    ...
    async fn set_model_updates_profile() -> anyhow::Result<()> {
        ...
        ConfigEditsBuilder::new(codex_home.path())
            ...
            .set_model(Some("big-pickle"), Some(ReasoningEffort::Medium))
            .apply()
            .await?;
        ...
        assert_eq!(profile.model.as_deref(), Some("big-pickle"));
        assert_eq!(
