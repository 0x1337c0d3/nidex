...
///
/// ```rust
/// codex_core::feedback_tags!(model = "big-pickle", cached = true);
...
mod tests {
  ...
  "error": {
    ...
    fn feedback_tags_macro_compiles() {
        ...
        feedback_tags!(model = "big-pickle", cached = true, debug_only = OnlyDebug);
    }
    ...
    fn resume_command_prefers_name_over_id() {
        ...
        assert_eq!(command, Some("nidex resume my-thread".to_string()));
    }
    ...
    fn resume_command_quotes_thread_name_when_needed() {
        ...
        assert_eq!(
            command,
            Some("nidex resume -- -starts-with-dash".to_string())
        );

        let command = resume_command(Some("two words"), None);
        assert_eq!(command, Some("nidex resume 'two words'".to_string()));

        let command = resume_command(Some("quote'case"), None);
        assert_eq!(command, Some("nidex resume \"quote'case\"".to_string()));
    }
}
