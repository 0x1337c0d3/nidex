...
mod tests {
    ...
    fn assert_seatbelt_denied(stderr: &[u8], path: &Path) {
        ...
        assert!(
            stderr.contains(&path_str) && stderr.contains("Operation not permitted")
...
"#,
    ...
    fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
        ...
        fs::create_dir_all(vulnerable_root.join(".nidex")).expect("create .codex");
        fs::write(
            vulnerable_root.join(".nidex").join("config.toml"),
            "sandbox_mode = \"read-only\"\n",
        )
        ...
        let dot_codex_canonical = vulnerable_root_canonical.join(".nidex");
