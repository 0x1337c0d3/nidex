...
impl ShellHandler {
    fn to_exec_params(params: &ShellToolCallParams, turn_context: &TurnContext) -> ExecParams {
        let command = if params.command.len() == 1 {
            // Parse single command string using shlex to split into proper arguments
            shlex::split(&params.command[0]).unwrap_or_else(|| params.command.clone())
        } else {
...
impl ToolHandler for ShellHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        ...
        match payload {
            ToolPayload::Function { arguments } => {
                tracing::debug!(arguments = %arguments, "shell tool call from LLM");
...
impl ToolHandler for ShellCommandHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<ToolOutput, FunctionCallError> {
        ...
        tracing::debug!(arguments = %arguments, "shell_command tool call from LLM");
...
impl ShellHandler {
    async fn run_exec_like(args: RunExecLikeArgs) -> Result<ToolOutput, FunctionCallError> {
        ...
        tracing::debug!(command = ?exec_params.command, cwd = ?exec_params.cwd, "executing shell command");
