...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".nidex")).await?;
    tokio::fs::create_dir_all(project_root.join(".nidex")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".nidex").join(CONFIG_TOML_FILE),
        "foo = \"root\"\n",
    )
    .await?;
    tokio::fs::write(
        nested.join(".nidex").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;
    ...
    assert_eq!(project_layers[0].as_path(), nested.join(".nidex").as_path());
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".nidex").as_path()
    );
...
"#;
    tokio::fs::write(project_root.join(".nidex").join(CONFIG_TOML_FILE), root_cfg).await?;
    tokio::fs::write(nested.join(".nidex").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".nidex").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".nidex").join("child.txt"),
        "child instructions",
    )
    .await?;
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_codex_folder: AbsolutePathBuf::from_absolute_path(project_root.join(".nidex"))?,
            },
            config: TomlValue::Table(toml::map::Map::new()),
            version: version_for_toml(&TomlValue::Table(toml::map::Map::new())),
            disabled_reason: None,
...
async fn codex_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    ...
    let codex_home = home_dir.join(".nidex");
    tokio::fs::create_dir_all(&codex_home).await?;
...
async fn codex_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    ...
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_codex = project_root.join(".nidex");
    let nested_dot_codex = nested.join(".nidex");
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".nidex")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".nidex").join(CONFIG_TOML_FILE), "foo =").await?;
