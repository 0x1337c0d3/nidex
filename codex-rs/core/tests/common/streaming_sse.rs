                ...
                accept_res = listener.accept() => {
                    ...
                    tokio::spawn(async move {
                        ...
                        if method == "POST" && path == "/v1/responses" {
                            let raw = match read_request_body(&mut stream, &request, body_prefix)
                                .await
                            {
                                Ok(body) => body,
                                Err(_) => {
                            ...
                            let body = if content_encoding(&request)
                                .is_some_and(|enc| enc.eq_ignore_ascii_case("zstd"))
                            {
                                zstd::stream::decode_all(std::io::Cursor::new(raw))
                                    .unwrap_or_else(|e| panic!("zstd decode failed: {e}"))
                            } else {
                                raw
                            };
...
fn content_encoding(headers: &str) -> Option<String> {
