...
impl TestCodexHarness {
    ...
    pub async fn request_bodies(&self) -> Vec<Value> {
        use crate::responses::decode_body_bytes;
    ...
    pub async fn function_call_stdout(&self, call_id: &str) -> String {
        let output_val = self.function_call_output_value(call_id).await;
    ...
    pub async fn custom_tool_call_output(&self, call_id: &str) -> String {
        let bodies = self.request_bodies().await;
        let output_val = custom_tool_call_output(&bodies, call_id);
        // Try content field first (new format), then output field (old format)
        output_val
            .get("content")
            .and_then(Value::as_str)
