...
pub fn is_zstd_encoding(value: &str) -> bool {
    value
        .split(',')
        .any(|entry| entry.trim().eq_ignore_ascii_case("zstd"))
}

pub fn decode_body_bytes(body: &[u8], content_encoding: Option<&str>) -> Vec<u8> {
...
impl ResponsesRequest {
    ...
    pub fn function_call_output_text(&self, call_id: &str) -> Option<String> {
        ...
        // Try content field first (new format), then output field (old format)
        item.get("content")
    ...
    fn call_output_content_and_success(
        ...
        // New format: "content" and "success" fields at top level
        if let Some(content_val) = item.get("content") {
            let content = match content_val {
                Value::String(s) => Some(s.clone()),
                Value::Object(obj) => obj.get("content")
                    .and_then(Value::as_str)
                    .map(str::to_string),
                _ => None,
            };
        ...
        // Old format: "output" field which could be string or {content, success} object
        let output = item
            .get("output")
            .cloned()
...
/// Wiremock matcher that decompresses zstd request bodies before string matching.
/// Use instead of `body_string_contains` since request bodies are zstd-compressed
/// by default (EnableRequestCompression is on).
pub struct DecodedBodyStringContains(String);

impl Match for DecodedBodyStringContains {
    fn matches(&self, request: &wiremock::Request) -> bool {
        let body = decode_body_bytes(
            &request.body,
            request
        ...
        String::from_utf8_lossy(&body).contains(self.0.as_str())
    }
}

pub fn decoded_body_string_contains(s: impl Into<String>) -> DecodedBodyStringContains {
    DecodedBodyStringContains(s.into())
}
...
pub async fn start_mock_server() -> MockServer {
    let server = MockServer::builder()
        .body_print_limit(BodyPrintLimit::Limited(200))
        .start()
        .await;
