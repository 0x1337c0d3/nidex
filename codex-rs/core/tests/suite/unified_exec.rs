...
fn extract_output_text(item: &Value) -> Option<&str> {
    item.get("content").and_then(|value| match value {
...
async fn unified_exec_emits_exec_command_end_event() -> Result<()> {
    ...
    // ExecCommandEnd may arrive after TurnComplete when the background exit
    // watcher fires post-turn; collect both in any order.
    ...
    loop {
        ...
        if turn_complete && end_event.is_some() {
            break;
        }
    }
    let end_event = end_event.unwrap();
...
async fn unified_exec_emits_one_begin_and_one_end_event() -> Result<()> {
    ...
    loop {
        ...
        // ExecCommandEnd is emitted by a background watcher that may fire after
        // TurnComplete (the watcher waits for output_drained before emitting).
...
#[cfg(target_os = "macos")]
#[ignore = "flaky under parallel test load: python TTY output misses 1500ms yield window"]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
