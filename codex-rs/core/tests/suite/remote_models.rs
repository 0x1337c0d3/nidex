...
#[ignore = "truncation policy override behavior changed; needs test update"]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
...
async fn remote_models_apply_remote_base_instructions() -> Result<()> {
    ...
    let model = "test-big-pickle-remote";
    ...
    let base_model_info = models_manager.get_model_info("big-pickle", &config).await;
...
async fn remote_models_preserve_builtin_presets() -> Result<()> {
    ...
    assert!(
        available
            .iter()
            .any(|model| model.model == "big-pickle"),
