...
fn configure_shell_model(
    ...
    let builder = match (output_type, include_apply_patch_tool) {
        (ShellModelOutput::ShellCommand, _) => builder.with_model("test-big-pickle-codex"),
        (ShellModelOutput::LocalShell, true) => builder.with_model("big-pickle"),
        (ShellModelOutput::Shell, true) => builder.with_model("big-pickle"),
        (ShellModelOutput::LocalShell, false) => builder.with_model("codex-mini-latest"),
        (ShellModelOutput::Shell, false) => builder.with_model("big-pickle"),
    };
...
#[ignore = "shell JSON output format changed; needs test update"]
