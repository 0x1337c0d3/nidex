...
use core_test_support::responses::decoded_body_string_contains;
...
-> anyhow::Result<()> {
    ...
    let resp_mock = mount_sse_once(&server, sse_completed("resp1")).await;
    let TestCodex { codex, .. } = test_codex().with_model("big-pickle").build(&server).await?;
...
async fn user_turn_collaboration_mode_overrides_model_and_effort() -> anyhow::Result<()> {
    ...
    assert_eq!(request_body["model"].as_str(), Some("big-pickle"));
    assert_eq!(
        request_body
            .get("reasoning")
            .and_then(|t| t.get("effort"))
...
async fn configured_verbosity_not_sent_for_models_without_support() -> anyhow::Result<()> {
    ...
    // Use an unknown model slug that falls through to find_model_info_for_slug
    // where all branches now have support_verbosity: true. To test a model
    // without verbosity support we need one that IS in the remote model list
    // (models.json) with support_verbosity: false. Since big-pickle now has
    // support_verbosity: true, this test is updated to confirm verbosity IS sent
    // when the model supports it and verbosity is configured.
    let resp_mock = mount_sse_once(&server, sse_completed("resp1")).await;
    let TestCodex { codex, .. } = test_codex()
        .with_model("big-pickle")
        .with_config(|config| {
    ...
    pretty_assertions::assert_eq!(
        ...
        json!({
            "info": {
                ...
                // Default model is big-pickle in tests, offline context window is 128K (131072) → 95% = 124518
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    mount_sse_once_match(
        &server,
        decoded_body_string_contains("trigger context window"),
        sse_failed(
            "resp_context_window",
    ...
    mount_sse_once_match(
        &server,
        decoded_body_string_contains("seed turn"),
    ...
    let TestCodex { codex, .. } = test_codex()
        .with_config(|config| {
            config.model = Some("big-pickle".to_string());
