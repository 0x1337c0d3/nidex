...
async fn truncate_function_error_trims_respond_to_model() -> Result<()> {
    ...
    let mut builder = test_codex().with_model("test-big-pickle-codex");
    ...
    assert!(
        ...
        "error output should not include line-based truncation header: {}",
        &output[..output.len().min(50)],
    );
    let truncated_pattern = r"(?s)^unable to access `.*chars truncated.*$";
    assert_regex_match(truncated_pattern, &output);
    assert!(
        !output.contains("omitted"),
        "line omission marker should not appear when no lines were dropped: {}",
        &output[..output.len().min(50)],
    );
...
#[ignore = "truncation policy behavior changed; needs test update"]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn tool_call_output_configured_limit_chars_type() -> Result<()> {
    ...
    let mut builder = test_codex().with_model("codex-test").with_config(|config| {
...
async fn tool_call_output_exceeds_limit_truncated_chars_limit() -> Result<()> {
    ...
    // Use a model with bytes policy that exposes the shell_command tool.
    let mut builder = test_codex().with_model("codex-test");
...
async fn tool_call_output_exceeds_limit_truncated_for_model() -> Result<()> {
    ...
    // Use a model that exposes the shell_command tool.
    let mut builder = test_codex().with_model("big-pickle");
    let fixture = builder.build(&server).await?;
...
async fn mcp_image_output_preserves_image_and_no_text_summary() -> Result<()> {
    ...
    let output = output_item.get("content").expect("output");
...
async fn token_policy_marker_reports_tokens() -> Result<()> {
    ...
    let server = start_mock_server().await;
    let mut builder = test_codex().with_model("big-pickle").with_config(|config| {
...
#[ignore = "truncation output format changed; needs test update"]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
