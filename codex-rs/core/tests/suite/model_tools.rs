...
async fn model_selects_expected_tools() {
    ...
    assert_eq!(
        ...
        "codex-mini-latest should expose the shell_command tool",
    );

    let gpt5_codex_tools = collect_tool_identifiers_for_model("big-pickle-codex").await;
    assert_eq!(
        gpt5_codex_tools,
        vec![
            "shell_command".to_string(),
        ...
        "big-pickle-codex should expose the shell_command tool",
    );

    let gpt51_codex_tools = collect_tool_identifiers_for_model("big-pickle").await;
    assert_eq!(
        gpt51_codex_tools,
        vec![
            "shell_command".to_string(),
        ...
        "big-pickle should expose the apply_patch tool",
    );
    ...
    assert_eq!(
        ...
        "exp-5.1 should expose the shell_command tool",
    );
}
