...
// When flattened into FunctionCallOutput, this struct's fields become:
//   - content: the tool result (string or array)
//   - success: optional boolean flag (only serialized if Some)
impl Serialize for FunctionCallOutputPayload {
        ...
        S: Serializer,
    {
        ...
        // Calculate field count: always have content, plus success if present
        ...
        // Serialize success field if present
...
- ["a", "b", "c"]"#
    ...
    fn serializes_success_as_plain_string() -> Result<()> {
        ...
        // With flatten, fields are at the root level
        assert_eq!(v.get("call_id").unwrap().as_str().unwrap(), "call1");
        assert_eq!(v.get("content").unwrap().as_str().unwrap(), "ok");
        // success is not serialized when None
    ...
    fn serializes_failure_as_string() -> Result<()> {
        ...
        // With flatten, fields are at the root level
        assert_eq!(v.get("call_id").unwrap().as_str().unwrap(), "call1");
        assert_eq!(v.get("content").unwrap().as_str().unwrap(), "bad");
        assert_eq!(v.get("success").unwrap().as_bool().unwrap(), false);
        Ok(())
    }
    ...
    fn serializes_image_outputs_as_array() -> Result<()> {
        ...
        // With flatten, content field is at the root level
        let content = v.get("content").expect("content field");
        assert!(content.is_array(), "expected array content");
