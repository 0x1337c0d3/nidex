    ...
    pub fn new_or_append(
        ...
        let mut info = match info {
            Some(info) => {
                ...
                // Always update model_context_window to ensure it is set correctly.
                if let Some(window) = model_context_window {
                    info.model_context_window = Some(window);
                }
                info
            }
            None => Self {
...
impl TokenUsage {
    ...
    /// `BASELINE_TOKENS` is subtracted from the denominator to define the "effective window"
    /// — the portion users can actually fill. Token counts (from both the API and our estimator)
    /// already include system-prompt overhead, so we do not subtract baseline from the numerator.
