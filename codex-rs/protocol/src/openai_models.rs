...
/// Default message roles supported by most models.
/// Only includes roles universally supported (excludes "developer" which is provider-specific).
fn default_supported_message_roles() -> Vec<String> {
    vec![
        "system".to_string(),
        "user".to_string(),
        "assistant".to_string(),
...
pub struct ModelInfo {
    ...
    #[serde(default = "default_input_modalities")]
    pub input_modalities: Vec<InputModality>,
    /// Message roles supported by this model (e.g., "system", "user", "assistant", "tool", "developer").
    #[serde(default = "default_supported_message_roles")]
    pub supported_message_roles: Vec<String>,
    /// Field name used for reasoning content in API requests/responses.
    /// Detected dynamically from responses. If None, defaults to "reasoning".
    /// Some providers (e.g., DeepSeek) use "reasoning_content" instead.
    #[serde(default, skip_serializing_if = "Option::is_none")]
