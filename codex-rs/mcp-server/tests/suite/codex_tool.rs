...
async fn codex_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // The mock model does not support the "developer" role, so developer
    // instructions are mapped to the "system" role fallback.
    let system_messages: Vec<&serde_json::Value> = request["messages"]
        .as_array()
        .unwrap()
        .iter()
        .filter(|msg| msg.get("role").and_then(|role| role.as_str()) == Some("system"))
        .collect();
    ...
    assert!(
        ...
        "expected permissions instruction, got {system_contents:?}"
    );
    assert!(
        system_contents.contains(&"Foreshadow upcoming tool calls."),
        "expected developer instructions in system messages, got {system_contents:?}"
    );
