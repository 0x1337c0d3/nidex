...
async fn test_list_and_resume_conversations() -> Result<()> {
    ...
    // May contain TokenCount events alongside user messages
    let user_messages: Vec<_> = session_initial_messages
        .iter()
    ...
    assert_eq!(user_messages[0].message, first_item.preview.clone());
    let resume_resp: JSONRPCResponse = timeout(
        DEFAULT_READ_TIMEOUT,
    ...
    // May contain TokenCount events alongside user messages
    let user_messages: Vec<_> = by_id_initial_messages
        .iter()
        .filter_map(|msg| match msg {
            EventMsg::UserMessage(msg) => Some(msg),
            _ => None,
