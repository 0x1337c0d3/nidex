    ...
    let model_provider = if oss {
        ...
        if let Some(provider) = resolved {
            ...
            return Err(anyhow::anyhow!(
                "No default OSS provider configured. Use --local-provider=provider or set oss_provider to one of: {OLLAMA_OSS_PROVIDER_ID}, {OLLAMA_CHAT_PROVIDER_ID} in config.toml"
            ));
        }
    } else {
