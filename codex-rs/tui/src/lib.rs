...
pub async fn run_main(
    ...
    run_ratatui_app(cli, config, overrides, cli_kv_overrides)
        .await
...
async fn load_config_or_exit(
    ...
    load_config_or_exit_with_fallback_cwd(cli_kv_overrides, overrides, None)
        .await
}
...
mod tests {
    ...
    fn build_turn_context(config: &Config, cwd: PathBuf) -> TurnContextItem {
        let model = config
            .model
            .clone()
            .unwrap_or_else(|| "big-pickle".to_string());
        TurnContextItem {
            cwd,
