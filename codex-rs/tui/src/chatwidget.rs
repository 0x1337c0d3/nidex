...
const NUDGE_MODEL_SLUG: &str = "big-pickle";
...
impl ChatWidget {
    ...
    pub(crate) fn set_token_info(&mut self, info: Option<TokenUsageInfo>) {
        debug!("set_token_info called with info: {:?}", info.is_some());
        match info {
            Some(info) => self.apply_token_info(info),
            None => {
                debug!("set_token_info: info is None, clearing context window");
    ...
    fn context_used_tokens(&self, info: &TokenUsageInfo, percent_known: bool) -> Option<i64> {
        if percent_known {
            debug!("context_used_tokens: percent_known=true, returning None");
            return None;
        }

        let tokens = info.last_token_usage.tokens_in_context_window();
        debug!(
            "context_used_tokens: percent_known=false, tokens={}",
            tokens
        );
        Some(tokens)
    }
    ...
    pub(crate) fn new(common: ChatWidgetInit, thread_manager: Arc<ThreadManager>) -> Self {
        ...
        widget
            .bottom_pane
            .set_connectors_enabled(false /* Apps feature removed */);
    ...
    fn dispatch_event_msg(&mut self, id: Option<String>, msg: EventMsg, from_replay: bool) {
        ...
        match msg {
            ...
            EventMsg::TokenCount(ev) => {
                debug!(
                    "DEBUG dispatch_event_msg: TokenCount event received, info={}",
                    if ev.info.is_some() { "Some" } else { "None" }
                );
                if let Some(info) = &ev.info {
                    debug!(
                        "DEBUG TokenCount: model_context_window={:?}, tokens={}",
    ...
    fn prefetch_connectors(&mut self) {
        ...
        tokio::spawn(async move {
            let result = Ok(ConnectorsSnapshot {
                connectors: Vec::new(),
            });
    ...
    pub(crate) fn open_reasoning_popup(&mut self, preset: ModelPreset) {
        ...
        let warn_for_model = preset.model.starts_with("big-pickle")
            || preset.model.starts_with("big-pickle")
            || preset.model.starts_with("big-pickle");
    ...
    fn open_connectors_popup(&mut self, connectors: &[AppInfo]) {
        ...
        for connector in connectors {
            let connector_label = connector.display_name.clone();
            ...
            let (selected_label, missing_label, instructions) = if connector.is_accessible {
                (
                    "Press Enter to view the app link.".to_string(),
                    "App link unavailable.".to_string(),
                    "Manage this app in your browser.".to_string(),
                )
            } else {
                (
                    "Press Enter to view the install link.".to_string(),
                    "Install link unavailable.".to_string(),
                    "Install this app in your browser, then reload Codex.".to_string(),
                )
            };
            if let Some(install_url) = connector.install_url.clone() {
                ...
                item.selected_description = Some(selected_label.clone());
            } else {
                let missing_label_clone = missing_label.clone();
                item.actions = vec![Box::new(move |tx| {
                    tx.send(AppEvent::InsertHistoryCell(Box::new(
                        history_cell::new_info_event(missing_label_clone.clone(), None),
                    )));
                })];
                item.dismiss_on_select = true;
                item.selected_description = Some(missing_label.clone());
            }
            items.push(item);
        }
