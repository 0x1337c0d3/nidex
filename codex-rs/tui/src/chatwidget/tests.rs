...
async fn review_restores_context_window_indicator() {
    ...
    let pre_review_tokens = 700; // ~30% remaining after subtracting baseline.
    let review_tokens = 30; // ~97% remaining after subtracting baseline.
...
async fn rate_limit_switch_prompt_shows_once_per_session() {
    ...
    let (mut chat, _, _) = make_chatwidget_manual(Some("big-pickle-codex")).await;
...
async fn rate_limit_switch_prompt_popup_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("big-pickle-codex")).await;
    chat.auth_manager =
        AuthManager::from_auth_for_testing(CodexAuth::create_dummy_chatgpt_auth_for_testing());
...
async fn plan_implementation_popup_yes_emits_submit_message_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("big-pickle-codex")).await;
    chat.open_plan_implementation_prompt();
...
async fn submit_user_message_with_mode_sets_coding_collaboration_mode() {
    let (mut chat, _rx, mut op_rx) = make_chatwidget_manual(Some("big-pickle-codex")).await;
...
async fn set_model_updates_active_collaboration_mask() {
    ...
    chat.set_model("big-pickle");

    assert_eq!(chat.current_model(), "big-pickle");
    assert_eq!(chat.active_collaboration_mode_kind(), ModeKind::Plan);
}
...
async fn model_reasoning_selection_popup_snapshot() {
    ...
    let preset = get_available_model(&chat, "big-pickle");
