...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    ...
    metrics.counter("codex.turns", 1, &[("model", "big-pickle"), ("env", "dev")])?;
    ...
    let expected_counter_attributes = BTreeMap::from([
        ...
        ("model".to_string(), "big-pickle".to_string()),
    ]);
...
fn client_sends_enqueued_metric() -> Result<()> {
    ...
    metrics.counter("codex.turns", 1, &[("model", "big-pickle")])?;
    metrics.shutdown()?;
    ...
    assert_eq!(attrs.get("model").map(String::as_str), Some("big-pickle"));
