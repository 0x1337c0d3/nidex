...
fn exec_or_panic(command: Vec<String>) -> ! {
    // Use shell interpretation for single command strings (common from tool calls)
    // If it's a single string, it may contain arguments that need shell parsing
    if command.len() == 1 {
        let shell_cmd = &command[0];
        let c_sh = CString::new("/bin/sh").expect("Failed to convert /bin/sh to CString");
        let c_flag = CString::new("-c").expect("Failed to convert -c to CString");
        let c_cmd = CString::new(shell_cmd.as_str()).expect("Failed to convert shell command to CString");
        ...
        panic!("Failed to execvp /bin/sh -c '{shell_cmd}': {err}");
    }

    // Standard path: execute command directly with separate pre-parsed arguments
